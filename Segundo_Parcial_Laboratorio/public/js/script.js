...
function manejadorAlta(e) {
  ...
  anuncios = Controller.alta(anuncios);
...
function mostrar() {
  ...
  selectores.map(x =>{
    uncheckedBox.each( elemento => {
      if(uncheckedBox[elemento].id == selectores[x].id.substring(4).toLowerCase())
      {
...
function CrearAnuncios(datosJSON) {
    ...
    {
      datosJSON.forEach(elemento => listaAnuncios.push(new BienRaiz(elemento.id, elemento.titulo, elemento.descripcion,
      ...
      switch (selector) {
        case "sel_transaccion":
          datosFiltrados = datosFiltrados.filter(elemento => {
            return elemento.transaccion.toLowerCase() === selectores[indice].value;
          });
          break;
        case "sel_precio": 
          datosFiltrados = datosFiltrados.filter(elemento => elemento.precio <= selectores[indice].value);
          PromedioPrecio(CrearAnuncios(datosFiltrados));
          break;
        case "sel_dormitorio": 
...
function PromedioPrecio(listado)
{
  let promedio = listado.map(elemento => parseInt(elemento.getPrecio)).
                                      reduce((prev,curr) => (prev + curr)) / listado.length;
  $("#promedioPrecio").val(promedio);
}
